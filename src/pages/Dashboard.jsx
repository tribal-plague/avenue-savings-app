    ...
    <div className="max-w-7xl mx-auto px-6 py-8 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-6xl font-bold text-avenue-dark leading-none tabular-nums">
          ...
          <p className="text-sm text-avenue-muted mt-2">
            ...
            {monthChange !== 0 && (
              <span className={`ml-2 font-medium ${monthChange > 0 ? 'text-red-500' : 'text-avenue-dark'}`}>
        ...
        <div className="flex gap-8 flex-shrink-0">
          <div>
            ...
            <p className="text-2xl font-bold text-avenue-dark tabular-nums">{salary > 0 ? fmtCurrency(salary) : '—'}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-avenue-muted uppercase tracking-widest mb-1">Savings Goal</p>
            <div className="flex items-center gap-2">
              <p className="text-2xl font-bold text-avenue-dark tabular-nums">{fmtCurrency(savingsTarget)}</p>
              {salary > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-avenue-border p-5">
            ...
            {topCats.length === 0 ? (
              ...
              <div>
                ...
                {(() => {
                  const uncatTotal = groupExpenses
                    .filter((e) => {
                      const d = new Date(e.date + 'T00:00:00')
                      return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth() && !e.categoryId
                    })
                    .reduce((acc, e) => acc + e.amount, 0)
                  return uncatTotal > 0 ? (
                    <div className="flex items-center gap-3 py-2.5 border-t border-avenue-border mt-1">
                      <div className="w-2 h-2 rounded-full bg-avenue-border flex-shrink-0" />
                      <span className="flex-1 text-sm text-avenue-muted">Uncategorized</span>
                      <span className="text-sm font-semibold text-avenue-muted w-20 text-right tabular-nums">{fmtCurrency(uncatTotal)}</span>
                    </div>
                  ) : null
                })()}
              </div>
          ...
          {/* Avenue Analysis */}
          <div className="bg-avenue-dark rounded-xl p-5">
            ...
            <div className="space-y-3">
              {insights.slice(0, 3).map((ins, i) => (
                <div key={i} className={`rounded-lg p-3.5 ${ins.type === 'success' ? 'bg-white/8 border border-white/10' : ins.type === 'warning' ? 'bg-white/8 border border-white/10' : 'bg-white/6 border border-white/8'}`}>
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-xs leading-relaxed">{ins.body}</p>
                    </div>
                  </div>
                </div>
              ))}
              {suggestions[0] && (
                <div className="mt-1 p-3 border border-white/10 bg-white/6 rounded-lg">
                  ...
                  <p className="text-white/40 text-xs leading-relaxed">{suggestions[0].body}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-avenue-border p-5">
            ...
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={trendData}>
                ...
                <Tooltip formatter={(v) => [fmtCurrency(v), 'Spent']} contentStyle={{ borderRadius: 8, border: '1px solid #E4E4E7', fontSize: 12, backgroundColor: '#fff' }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-xl border border-avenue-border p-5">
              ...
              <div className="space-y-3">
                    ...
                    <div key={m.id}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: m.avatarColor || '#0D2E3F' }}>
                            {(m.name || '?')[0]}
                          </div>
                          <div>
                ...
                {members.length === 1 && (
                  <p className="text-xs text-avenue-muted pt-1">You're the only member. <button onClick={() => setPage('groups')} className="text-avenue-dark font-medium hover:text-avenue-muted">Invite someone →</button></p>
                )}
              </div>
            </div>
                ...
                {recentExp.map((exp) => {
                  ...
                  return (
                    <div key={exp.id} className="flex items-center gap-3 py-2">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base flex-shrink-0 bg-avenue-surface">
                        {cat?.icon || '📦'}
                      </div>
